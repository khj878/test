...
void tmp_manual_mode();
void feed_manual_mode();
...
void excute_query()
{
	...
	while ((row = mysql_fetch_row(res)) != NULL){
		...
		{
			printf("Manual temperature control mode :: ");
			tmp_manual_mode();
		}
...
void tmp_manual_mode()
{
	...
	{
		digitalWrite(HEATPAD, 0);
		printf("Manual Heatpad On\n");
	}
	else
	{
		digitalWrite(HEATPAD, 1);
		printf("Manual Heatpad Off\n");
	}
}
...
void feed_manual_mode()
{
	...
	{
		survo_control();
		printf("feeder_once_open\n");
	}

	char query[30] = "update test set feed_btn = ";
	strcat(query, "0");
